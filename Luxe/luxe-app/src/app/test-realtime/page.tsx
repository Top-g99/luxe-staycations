  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      try {
        ...
        // Load data
        setProperties(propertyManager.getAll());
        setDestinations(destinationManager.getAll());
        setDealBanners(dealBannerManager.getAll());
        setHeroBackgrounds(heroBackgroundManager.getAll());
    ...
    const unsubscribeProperty = propertyManager.subscribe(() => {
      setProperties(propertyManager.getAll());
      setLastUpdate(new Date());
    });

    const unsubscribeDestination = destinationManager.subscribe(() => {
      setDestinations(destinationManager.getAll());
      setLastUpdate(new Date());
    });

    const unsubscribeDealBanner = dealBannerManager.subscribe(() => {
      setDealBanners(dealBannerManager.getAll());
      setLastUpdate(new Date());
    });

    const unsubscribeHeroBackground = heroBackgroundManager.subscribe(() => {
      setHeroBackgrounds(heroBackgroundManager.getAll());
      setLastUpdate(new Date());
    });
