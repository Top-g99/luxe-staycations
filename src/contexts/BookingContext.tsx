...
export function BookingProvider({ children }: { children: ReactNode }) {
  ...
  const updateBooking = (id: string, updates: Partial<CompleteBooking>) => {
    ...
    if (typeof window !== 'undefined') {
      import('@/lib/dataManager').then(async ({ bookingManager }) => {
        try {
          const existingBooking = await bookingManager.getById(id);
          if (existingBooking) {
            ...
            bookingManager.update(id, {
              ...
              property_id: updates.bookingDetails?.propertyId || existingBooking.property_id,
