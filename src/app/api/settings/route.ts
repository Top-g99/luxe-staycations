...
export async function GET(request: NextRequest) {
  try {
    ...
    let data;
    if (category) {
      data = await settingsManager.getSettingsByCategory(category as any);
    } else {
      data = {
        settings: await settingsManager.getAllSettings(),
        businessProfile: await settingsManager.getBusinessProfile(),
        paymentSettings: await settingsManager.getPaymentSettings(),
        notificationSettings: await settingsManager.getNotificationSettings(),
        securitySettings: await settingsManager.getSecuritySettings(),
        stats: await settingsManager.getSettingsStats()
      };
    }
  ...
  try {
    ...
    switch (action) {
      case 'updateSetting':
        ...
        const success = await settingsManager.updateSetting(data.key, data.value);
        return NextResponse.json({
          success,
      ...
      case 'updateBusinessProfile':
        const profileSuccess = await settingsManager.updateBusinessProfile(data);
        return NextResponse.json({
      ...
      case 'updatePaymentSettings':
        const paymentSuccess = await settingsManager.updatePaymentSettings(data);
        return NextResponse.json({
      ...
      case 'updateNotificationSettings':
        const notificationSuccess = await settingsManager.updateNotificationSettings(data);
        return NextResponse.json({
      ...
      case 'updateSecuritySettings':
        const securitySuccess = await settingsManager.updateSecuritySettings(data);
        return NextResponse.json({
