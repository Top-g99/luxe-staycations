          ...
          <Card sx={{ height: '100%', background: 'linear-gradient(135deg, #2E7D32 0%, #388E3C 100%)', color: 'white' }}>
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Box>
                  <Typography variant="h4" component="div" sx={{ fontWeight: 'bold' }}>
                    {host.properties.reduce((sum, prop) => sum + (prop.totalBookings || 0), 0)}
                  </Typography>
                  <Typography variant="body2" sx={{ opacity: 0.9 }}>
                    Total Bookings
                  </Typography>
          ...
          <Card sx={{ height: '100%', background: 'linear-gradient(135deg, #FF9800 0%, #FFB74D 100%)', color: 'white' }}>
            <CardContent>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Box>
                  <Typography variant="h4" component="div" sx={{ fontWeight: 'bold' }}>
                    {(host.verificationStatus || 'pending') === 'verified' ? '✓' : '⏳'}
                  </Typography>
                  <Typography variant="body2" sx={{ opacity: 0.9 }}>
                    {(host.verificationStatus || 'pending') === 'verified' ? 'Verified' : 'Pending'}
                  </Typography>
                </Box>
              ...
              <Card sx={{ height: '100%', position: 'relative' }}>
                <CardContent>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <Typography variant="body2" color="text.secondary" gutterBottom>
                      Base Price: {formatCurrency(property.pricing?.basePrice || property.price)}/night
                    </Typography>
                  </Box>
                    ...
                    <Grid item xs={6}>
                      <Box sx={{ textAlign: 'center' }}>
                        <Typography variant="h6" sx={{ fontWeight: 'bold', color: 'success.main' }}>
                          {formatCurrency(property.totalRevenue || 0)}
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
                          Total Revenue
                        </Typography>
                    ...
                    .map((property, index) => (
                      <ListItem key={property.id} sx={{ px: 0 }}>
                        ...
                        <ListItemText
                          ...
                          secondary={`${formatCurrency(property.totalRevenue || 0)} • ${property.totalBookings || 0} bookings`}
                        />
                      </ListItem>
                    ))}
                </List>
          ...
          <Grid item xs={12} md={6}>
            <Card>
              <CardContent>
                ...
                <Box sx={{ mb: 2 }}>
                  <Typography variant="body2" color="text.secondary">
                    Account: {host.bankDetails?.accountNumber || 'Not provided'}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    IFSC: {host.bankDetails?.ifscCode || 'Not provided'}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    Holder: {host.bankDetails?.accountHolderName || 'Not provided'}
                  </Typography>
                </Box>
                <Button variant="outlined" size="small">
                  Update Bank Details
