...
export default function InstagramFeed({ 
  ...
  const loadPosts = async () => {
    try {
      ...
      if (!isConfigured) {
        setError('Instagram integration not configured');
        setLoading(false);
        return;
      }
      ...
      if (result.success) {
        ...
        setError(result.message || 'Failed to load Instagram posts');
      }
    } catch (err) {
            ...
            >
              ...
              <CardContent>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                  ...
                  <Chip
                    icon={<Comment />}
                    label={formatNumber(post.commentsCount)}
                    size="small"
                    color="primary"
                    variant="outlined"
                  />
          ...
          <>
            ...
            <DialogContent>
              ...
              <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
                <Chip
                  icon={<Favorite />}
                  label={`${formatNumber(selectedPost.likeCount)} likes`}
                  color="error"
                  variant="outlined"
                />
                <Chip
                  icon={<Comment />}
                  label={`${formatNumber(selectedPost.commentsCount)} comments`}
                  color="primary"
                  variant="outlined"
                />
              </Box>
